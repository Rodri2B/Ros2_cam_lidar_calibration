...

{
    ...
    {   
        ...
        glBindBufferRange(GL_UNIFORM_BUFFER, DebugBoardBinding, uboDebugBoard, 0, sizeof(gpu_lidar_detected_board_buffer));
    ...
    if (Lidar_context == nullptr || Camera_context == nullptr || Lidar_inliers_context == nullptr)
...
void LidarCamSim::Scene::InitCameras(const ViewportCamConfig &view_cfg,const VirtualCamConfig &vir_cfg){
    ...
    ModelsVector["cam_model"] = std::make_shared<Model>("../resources/models/camera/camera2.obj");
    ...
    glBufferData(GL_PIXEL_PACK_BUFFER, camera_buffer_size, nullptr, GL_STREAM_READ); //camera_buffer_size*sizeof(float)
...
void LidarCamSim::Scene::AutoRenderVirtual(){
    ...
    for (auto it = ObjectsVector.begin(); it != ObjectsVector.end(); it++) {
        ...
        if(it->second->cameras_for_render.find("virtual_cam") != it->second->cameras_for_render.end() && it->second->auto_render){
            if(it->second->cameras_for_render["virtual_cam"]){
...
void LidarCamSim::Scene::draw_virtual_camera_settings(){
    ...
    if(ImGui::DragFloat("##rdistor1", &plumb_bob_cam->radialDistortionParams.x, 0.01f,-FLT_MAX,FLT_MAX,"K1: %.3f",ImGuiSliderFlags_AlwaysClamp))modfied_dist = true;
    ImGui::SameLine();
    if(ImGui::DragFloat("##rdistor2", &plumb_bob_cam->radialDistortionParams.y, 0.01f,-FLT_MAX,FLT_MAX,"K2: %.3f",ImGuiSliderFlags_AlwaysClamp))modfied_dist = true;
    ImGui::SameLine();
    if(ImGui::DragFloat("##rdistor3", &plumb_bob_cam->radialDistortionParams.z, 0.01f,-FLT_MAX,FLT_MAX,"K3: %.3f",ImGuiSliderFlags_AlwaysClamp))modfied_dist = true;
    ImGui::PushItemWidth((size.x-spacing_x)/2.0f);
    ImGui::Text("Tangential Distortion");
    if(ImGui::DragFloat("##tdistor1", &plumb_bob_cam->tangentialDistortionParams.x, 0.01f,-FLT_MAX,FLT_MAX,"P1: %.3f",ImGuiSliderFlags_AlwaysClamp))modfied_dist = true;
    ImGui::SameLine();
    if(ImGui::DragFloat("##tdistor2", &plumb_bob_cam->tangentialDistortionParams.y, 0.01f,-FLT_MAX,FLT_MAX,"P2: %.3f",ImGuiSliderFlags_AlwaysClamp))modfied_dist = true;
    ImGui::PushItemWidth(size.x);
    ImGui::Text("Render Tile Size");
    ImGui::DragFloat("##rendertile", &plumb_bob_cam->tile_size, 1.0f,1.0f,FLT_MAX,"%.0f px",ImGuiSliderFlags_AlwaysClamp);
    ImGui::Text("Broadcast Options");
    ImGui::Checkbox("Broadcast",&camera_broadcast);
    ImGui::PopItemWidth();

    if(modfied_dist) plumb_bob_cam->resubmitDistortioncoeffs();
    if(modfied){
...
void LidarCamSim::Scene::draw_lidar_settings(){
    ...
    if(ImGui::Button("Apply",ImVec2(2.0f*button_size,button_size))){
        ...
        if(RecalculateLidarMutex.try_lock()){
            ...
            //LidarRecieveDataMutex.try_lock();
            LidarRecieveDataMutex.lock();

            lidar_inliers_recieved.resize(Lidar_dir_buffer_size+static_cast<uint32_t>(sizeof(lidar_detected_board)/sizeof(glm::vec3)));
            RecreateLidarInliersFIFO();

            lidar_ctrl_mtx->unlock();

            glBindVertexArray(LidarInliersVAO_main);
            glBindBuffer(GL_ARRAY_BUFFER, LidarInliersVBO);
            glBufferData(GL_ARRAY_BUFFER, Lidar_dir_buffer_size*sizeof(glm::vec3), NULL, GL_STATIC_DRAW);
            glBindBuffer(GL_ARRAY_BUFFER, 0);
            glBindVertexArray(0);
            inliers_recieved_first_time  = false;
            lidar_detected_board_recieved_first_time = false;
...
void LidarCamSim::Scene::draw_board_settings(){
    ...
    if(ImGui::Checkbox("View Board",&view_board)){
        if(view_board){
            ObjectsVector["chessboard"]->cameras_for_render["viewport_cam"] = true;
        }
        else{
            ObjectsVector["chessboard"]->cameras_for_render["viewport_cam"] = false;
        }
    }
...
void LidarCamSim::Scene::draw_statistics_viewport(){
    ...
    //if(ImGui::Checkbox("Enable Vsync##statistics",&enable_vsync)){
    //    if(enable_vsync){
    //        swap_interval = 1.0f;
    //        glfwSwapInterval(1);
    //    }else{
    ...
    //        
    //}
    //if(!enable_vsync) ImGui::BeginDisabled();
    //if(ImGui::DragFloat("##VsyncEnable", &swap_interval, 1.0f,0.0f,10.0f,"Swap Interval: %.0f",ImGuiSliderFlags_AlwaysClamp))
    //    glfwSwapInterval(static_cast<int>(swap_interval));
    //if(!enable_vsync) ImGui::EndDisabled();
    ImGui::NewLine();

    ImGui::Text("Lidar Debug Board");
    if(ImGui::Checkbox("Enable Debug Board##board_debug_lidar",&lidar_recieve_debug_board)){
        if(!lidar_recieve_debug_board){
            lidar_board_area_error = 0.0f;
            lidar_board_angle_error = 0.0f;
            lidar_board_position_error = 0.0f;
        }
    };
    if(!(lidar_recieve_debug_board && lidar_recieve_debug_cloud)) ImGui::BeginDisabled();
    ...
    ImGui::Text("Errors");
    if(!lidar_detected_board_recieved_first_time) ImGui::BeginDisabled();
    ImGui::PushItemWidth((size.x-spacing_x)/2.0f);
    ImGui::PushFont(font_mono);
    ImGui::SetWindowFontScale(0.85f);
    ImGui::InputFloat("##lboard_area_error", &lidar_board_area_error, 0.0f,0.0f,"Area: %.2f cm²",ImGuiInputTextFlags_ReadOnly);
    ImGui::SameLine();
    ImGui::InputFloat("##lboard_angle_error", &lidar_board_angle_error, 0.0f,0.0f, "Angle: %.1f°",ImGuiInputTextFlags_ReadOnly);
    ImGui::PushItemWidth(size.x);
    ImGui::InputFloat("##lposition_error", &lidar_board_position_error, 0.0f,0.0f, "Position: %.2f mm",ImGuiInputTextFlags_ReadOnly);
    ImGui::SetWindowFontScale(1.0f);
    ImGui::PopFont();
    if(!lidar_detected_board_recieved_first_time) ImGui::EndDisabled();
    if(!(lidar_recieve_debug_board && lidar_recieve_debug_cloud)) ImGui::EndDisabled();
    ImGui::PopItemWidth();
    //ImGui::PushItemWidth((size.x-spacing_x)/2.0f);
    //ImGui::InputFloat4("##l4m", line4, "%.3f",ImGuiInputTextFlags_ReadOnly);

    ImGui::Text("Camera Debug Board");
    if(ImGui::Checkbox("Enable Debug Board##board_debug_camera",&camera_recieve_debug_board)){
        if(!camera_recieve_debug_board){

            camera_board_area_error = 0.0f;
            camera_board_angle_error = 0.0f;
            camera_board_position_error = 0.0f;
        }
    };
    if(!camera_recieve_debug_board) ImGui::BeginDisabled();
    ...
    ImGui::Text("Errors");
    if(!camera_detected_board_recieved_first_time) ImGui::BeginDisabled();
    ImGui::PushItemWidth((size.x-spacing_x)/2.0f);
    ImGui::PushFont(font_mono);
    ImGui::SetWindowFontScale(0.85f);
    ImGui::InputFloat("##cboard_area_error", &camera_board_area_error, 0.0f,0.0f,"Area: %.2f cm²",ImGuiInputTextFlags_ReadOnly);
    ImGui::SameLine();
    ImGui::InputFloat("##cboard_angle_error", &camera_board_angle_error, 0.0f,0.0f, "Angle: %.1f°",ImGuiInputTextFlags_ReadOnly);
    ImGui::PushItemWidth(size.x);
    ImGui::InputFloat("##cposition_error", &camera_board_position_error, 0.0f,0.0f, "Position: %.2f mm",ImGuiInputTextFlags_ReadOnly);
    ImGui::SetWindowFontScale(1.0f);
    ImGui::PopFont();
    if(!camera_detected_board_recieved_first_time) ImGui::EndDisabled();
    if(!camera_recieve_debug_board) ImGui::EndDisabled();
    ImGui::PopItemWidth();

    ImGui::Text("Ground Truth Board");
    ImGui::Checkbox("View Normal##board_normal_truth",&render_ground_truth_normal);
...
void LidarCamSim::Scene::InitLidar(const Lidar_config &lidar_cfg,const glm::vec3 &Pos,const float &roll,const float &pitch,const float &yaw){
    ...
    ModelsVector["lidar_model"] = std::make_shared<Model>("../resources/models/lidar/lidar2.obj");
    ...
    point_shID = ShadersVector["lidar_inliers_shader"]->ID; 
    // then we link each shader's uniform block to this uniform binding point
    glUniformBlockBinding(point_shID, glGetUniformBlockIndex(point_shID, "Matrices_cam"), ProjViewBindingViewportCamera);

    glGenVertexArrays(1, &LidarInliersVAO_main);
    glGenBuffers(1, &LidarInliersVBO);
    glBindVertexArray(LidarInliersVAO_main);
    glBindBuffer(GL_ARRAY_BUFFER, LidarInliersVBO);
    glBufferData(GL_ARRAY_BUFFER, Lidar_dir_buffer_size*sizeof(glm::vec3), NULL, GL_STATIC_DRAW);
        ...
        if(camera_send_queue->get_max_msg() > camera_send_queue->get_num_msg() || virtual_camera_rescaled){
            ...
            {   
                ...
                if(rescaled){
                    ...
                    ////////////////////////// may be useless///////////////////////
                    CameraRecieveBoardMutex.lock(); // just not recreate recieve fifo
                    ///////////////////////////////////////////////////////////////
                    RecreateCameraFIFOS();

                    camera_ctrl_mtx->unlock();
                    ///////////////// may be useless/////////////////////////////
                    camera_detected_board_recieved_first_time = false;
                    CameraRecieveBoardMutex.unlock();
                    /////////////////////////////////////////////////////////

                    Camera_cpu_buffer.resize(camera_message_size);// /sizeof(float) //divide
                }
                //points_broadcast_buffer.resize(buffer_size);
                glBindBuffer(GL_PIXEL_PACK_BUFFER, camera_context_pbo);
                

                uint8_t* ptr = (uint8_t*)glMapBuffer(GL_PIXEL_PACK_BUFFER, GL_READ_ONLY);//float* ptr
                ...
                if (ptr) {
                    ...
                    uint32_t rowBytes   = width*3U;
                    for (uint32_t y = 0; y < height; y++) {
                        uint8_t* srcRow = ptr + (height - 1U - y)*rowBytes;
                        uint8_t* dstRow = Camera_cpu_buffer.data() + y*rowBytes;
    ...
    if(camera_recieve_debug_board || lidar_recieve_debug_board || render_ground_truth_normal){
        ...
        if(CameraRecieveBoardMutex.try_lock()){
            if(camera_detected_board_recieved_first_time && camera_recieve_debug_board){
                ...
                glm::mat4 pv_inverse_virtual = proj_view*plumb_bob_cam->camera_matrix;
...
void LidarCamSim::Scene::Draw_Lidar_inliers(){
    ...
    if(lidar_view_debug_cloud){
        ...
        if(LidarRecieveDataMutex.try_lock()){
            if(inliers_recieved_first_time){
                glBindVertexArray(LidarInliersVAO_main);
                glDrawArrays(GL_POINTS, 0, inliers_msg_recieve_size/sizeof(glm::vec3));
                glBindVertexArray(0);
            }
            LidarRecieveDataMutex.unlock();
        }
